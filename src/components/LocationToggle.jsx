...
function MalaysiaFlag({ className = "" }) {
  return (
    <svg viewBox="0 0 24 16" className={className} aria-hidden="true">
      <rect width="24" height="16" fill="#fff" />
      {[0, 2.29, 4.57, 6.86, 9.14, 11.43, 13.71].map((y) => (
...
/**
 * Location switcher — India is the default/active tab. Renders address,
 * phone (if provided) and flag per selected location.
 */
export default function LocationToggle({
  ...
  const locations = {
    india: { label: "India", flag: IndiaFlag, address: indiaAddress, phone: indiaPhone },
    malaysia: { label: "Malaysia", flag: MalaysiaFlag, address: malaysiaAddress, phone: malaysiaPhone },
  };
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium transition ${
                isActive ? "bg-brass text-charcoal" : "bg-white/10 text-mute hover:bg-white/20"
              }`}
            >
      ...
      <ul className="space-y-2 text-sm text-mute">
        ...
        {current.phone && (
          <li className="flex items-center gap-2">
            <Phone size={16} />
            <a href={`tel:${current.phone}`} className="hover:text-brass">
              {current.phone}
            </a>
          </li>
        )}
